...
import { Calendar as CalendarIcon, Car, Users, Navigation, Box, Shield, Baby, CircleDollarSign, ChevronRight } from 'lucide-react';
    ...
    <div className="w-full max-w-md mx-auto">
      <div className="bg-white rounded-2xl shadow-xl">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white p-6 rounded-t-2xl">
          ...
          <p className="text-blue-100 text-sm">Optimiert für Touch & Unterwegs</p>
        </div>
        ...
        <form className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="m-firstname" placeholder="Max" className="mt-1 h-12" />
              </div>
              <div>
                <Label htmlFor="m-lastname">Nachname</Label>
                <Input id="m-lastname" placeholder="Mustermann" className="mt-1 h-12" />
              </div>
              <div>
                <Label htmlFor="m-email">E-Mail</Label>
                <Input id="m-email" type="email" placeholder="max@beispiel.ch" className="mt-1 h-12" />
              </div>
              <div>
                <Label htmlFor="m-phone">Telefonnummer</Label>
                <Input id="m-phone" type="tel" placeholder="+41 79 123 45 67" className="mt-1 h-12" />
              </div>
            </div>
          </div>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        ...
                        {pickupDate ? format(pickupDate, 'dd.MM', { locale: de }) : 'Datum'}
                      </Button>
                    </PopoverTrigger>
                ...
                <div className="grid grid-cols-2 gap-2 mt-1">
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {returnDate ? format(returnDate, 'dd.MM', { locale: de }) : 'Datum'}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                ...
                return (
                  ...
                  >
                    <Checkbox
                      ...
                      className="w-5 h-5"
                    />
                    <Icon className={`w-6 h-6 ${selectedExtras.includes(extra.id) ? 'text-cyan-600' : 'text-gray-400'}`} />
                    <Label htmlFor={`m-${extra.id}`} className="flex-1 cursor-pointer">
                      {extra.label}
                    </Label>
                  </div>
                );
          ...
          <div className="space-y-4">
            ...
            <div className="bg-orange-50 p-4 rounded-lg space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-3xl text-orange-600">CHF {priceRange[0]}</span>
          ...
          <div className="space-y-4">
            ...
            <RadioGroup value={priority} onValueChange={setPriority}>
              <div className="space-y-2">
                <div className={`flex items-center space-x-3 p-4 rounded-lg border-2 min-h-[60px] ${priority === 'price' ? 'border-purple-500 bg-purple-50' : 'border-gray-200'}`}>
                  <RadioGroupItem value="price" id="m-price" className="w-5 h-5" />
                  <Label htmlFor="m-price" className="cursor-pointer flex-1">
                    <span className="block">💰 Preis</span>
                    <span className="text-xs text-gray-500">Bestes Angebot</span>
                  </Label>
                </div>
                <div className={`flex items-center space-x-3 p-4 rounded-lg border-2 min-h-[60px] ${priority === 'comfort' ? 'border-purple-500 bg-purple-50' : 'border-gray-200'}`}>
                  <RadioGroupItem value="comfort" id="m-comfort" className="w-5 h-5" />
                ...
                <div className={`flex items-center space-x-3 p-4 rounded-lg border-2 min-h-[60px] ${priority === 'sustainability' ? 'border-purple-500 bg-purple-50' : 'border-gray-200'}`}>
                  <RadioGroupItem value="sustainability" id="m-sustainability" className="w-5 h-5" />
                  <Label htmlFor="m-sustainability" className="cursor-pointer flex-1">
                    <span className="block">🌱 Nachhaltigkeit</span>
                    <span className="text-xs text-gray-500">Umweltfreundlich</span>
                  </Label>
                </div>
                <div className={`flex items-center space-x-3 p-4 rounded-lg border-2 min-h-[60px] ${priority === 'design' ? 'border-purple-500 bg-purple-50' : 'border-gray-200'}`}>
                  <RadioGroupItem value="design" id="m-design" className="w-5 h-5" />
      ...
      <div className="mt-6 bg-cyan-50 border border-cyan-200 rounded-lg p-4">
        <h4 className="text-cyan-700 mb-2">📱 Mobile-Layout Highlights</h4>
        <ul className="text-sm text-gray-700 space-y-1">
          <li>✓ Einspaltiges Layout für optimale Lesbarkeit (Luca: einhändige Bedienung)</li>
          <li>✓ Große Touch-Flächen (min. 44px) für präzise Eingabe unterwegs</li>
          <li>✓ Kompakte Kategoriewahl mit 3er-Grid</li>
          <li>✓ Preis prominent hervorgehoben (Sophie: Budget-Transparenz)</li>
          <li>✓ Gestaffelte Sektionen für klare Navigation</li>
          <li>✓ Sticky Submit-Button für schnellen Zugriff</li>
        </ul>
      </div>
    </div>
  );
